...
import { Icon, Flex, Modal, ModalBody, ModalHeader, ModalContent, ModalOverlay, Box, Button } from '@chakra-ui/react';

import { getUsersStats } from 'data';
...
export default function UsuariosInactivosModal({
  ...
  const copyUsuarios = () => {
    navigator.clipboard.writeText(
      usuarios?.reduce((acc, u) => (acc += (u.nombre || '') + ' ' + (u.apellidos || '') + ',' + u.email + '\n'), '')
    );
  };
      ...
      <ModalContent maxW="56em" maxH="56em">
        ...
        <ModalBody p="30px">
          <Flex direction="column" gap="30px">
            <Button onClick={copyUsuarios}>Copiar listado al portapapeles</Button>
            ...
            <Flex direction="column" w="100%" overflow="auto" maxH="500px">
              ...
              {usuarios?.map((u, index) => (
                <Flex key={'usuario-' + index} gap="20px">
                  <Box w="100%">{(u.nombre || '') + ' ' + (u.apellidos || '')}</Box>
